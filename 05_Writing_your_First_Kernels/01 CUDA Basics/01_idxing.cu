...
int main(int argc, char **argv) {
    int device_id = 0; // Set the GPU device ID here
    ...
    dim3 blocksPerGrid(2, 2, 1); // 3d cube of shape 2*3*4 = 24
    dim3 threadsPerBlock(4, 1, 1); // 3d cube of shape 4*4*4 = 64
    
    printf("Before the whoamai\n");
    whoami<<<blocksPerGrid, threadsPerBlock>>>();
    printf("After the whoamai\n");
